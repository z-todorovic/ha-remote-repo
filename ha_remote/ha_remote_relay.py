...
# === Configuration via environment or Supervisor autodetect ===
def discover_local_ha():
    ...
    return (
        os.getenv("HA_REMOTE_HA_HOST", "127.0.0.1"),
        int(os.getenv("HA_REMOTE_HA_PORT", "8123")),
    )


SERVER = os.getenv("HA_REMOTE_SERVER", "ws://localhost:8080/ha-cloud/ws/tunnel/test1")
...
# === Main loop with heartbeat/reconnect ===
async def main():
    reconnect_delay = 5
    while True:
        try:
            ...
            print(f"[HA Remote] Disconnected: {e}")
